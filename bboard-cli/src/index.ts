...
import { startApiServer, setDeployedAPI } from './api-server.js';
...
You can do one of the following:
  1. Deploy a new trucker load board contract
  2. Join an existing trucker load board contract
  3. Exit
...
const displayLedgerState = async (
  ...
  if (ledgerState === null) {
    logger.info(`No contract deployed at ${contractAddress}`);
  } else {
    const boardState = ledgerState.state === State.OCCUPIED ? 'occupied' : 'vacant';
    logger.info(`Board state: '${boardState}'`);
    logger.info(`Load number: '${ledgerState.loadNumber.is_some ? ledgerState.loadNumber.value : 'none'}'`);
...
const displayDerivedState = (state: BBoardDerivedState | undefined, logger: Logger) => {
  if (state === undefined) {
    logger.info(`No load board state currently available`);
  } else {
    const boardState = state.state === State.OCCUPIED ? 'occupied' : 'vacant';
    logger.info(`Board state: '${boardState}'`);
    logger.info(`Load number: '${state.loadNumber ?? 'none'}'`);
    logger.info(`Rate: ${state.rate} cents/mile ($${(Number(state.rate) / 100).toFixed(2)}/mile)`);
    logger.info(`Notes: '${state.notes ?? 'none'}'`);
    logger.info(`You are the shipper: ${state.isOwner}`);
  }
};
...
You can do one of the following:
  1. Post a load
  2. Claim a load (carrier)
  3. Take down your load (shipper only)
  4. Reveal your ownership (shipper only)
  5. Display the current ledger state (public)
  6. Display the current derived state (private)
  --- Carrier Registry ---
  7. Register as a carrier
  8. Verify carrier status
  9. Exit
Which would you like to do? `;
...
const mainLoop = async (providers: BBoardProviders, rli: Interface, logger: Logger): Promise<void> => {
  ...
  if (bboardApi === null) return;
  setDeployedAPI(bboardApi);

  let currentState: BBoardDerivedState | undefined;
  const subscription = bboardApi.state$.subscribe({ next: (state) => (currentState = state) });
  ...
  try {
    while (true) {
      ...
      try {
        switch (choice) {
          case '1': {
            const loadNumber = await rli.question('Load number (e.g. LOAD-2026-001): ');
            const rateStr = await rli.question('Rate in cents per mile (e.g. 250 = $2.50/mile): ');
            const notes = await rli.question('Notes (origin, destination, weight, etc.): ');
            await bboardApi.postLoad(loadNumber, BigInt(rateStr), notes);
            break;
          }
          case '2':
            await bboardApi.claimLoad();
            logger.info('Load claimed successfully!');
            break;
          case '3':
            await bboardApi.takeDown();
            logger.info('Load taken down successfully!');
            break;
          case '4':
            await bboardApi.revealOwnership();
            logger.info('Ownership revealed via ZK proof!');
            break;
          case '5':
          ...
          case '6':
            ...
            break;
          case '7':
            logger.info('registeringCarrier');
            await bboardApi.registerCarrier();
            logger.info('Registered as carrier successfully!');
            break;
          case '8':
            logger.info('verifyingCarrier');
            const proof = await bboardApi.verifyCarrier();
            logger.info('Carrier verified! Commitment: ' + proof);
            break;
          case '9':
            logger.info('Exiting...');
            return;
...
const buildWallet = async (config: Config, rli: Interface, logger: Logger): Promise<string | undefined> => {
  ...
  while (true) {
    ...
    switch (choice) {
      ...
      case '3': logger.info('Exiting...'); return undefined;
...
export const run = async (config: Config, testEnv: TestEnvironment, logger: Logger): Promise<void> => {
  ...
  try {
    ...
    const zkConfigProvider = new NodeZkConfigProvider<'postLoad' | 'claimLoad' | 'takeDown' | 'revealOwnership' | 'registerCarrier' | 'verifyCarrier'>(config.zkConfigPath);
